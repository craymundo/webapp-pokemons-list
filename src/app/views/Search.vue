<template>
  <div>
    <div v-if="pokemons.length === 0 && filter.length === 0">
      <div class="container">
    ...
    <div v-if="pokemons.length > 0 ||  filter.length > 0">
      ...
      <div class="footer">
        <Footer :is-disabled-favorites="true"
        :is-disabled-all="false"></Footer>
      </div>
    </div>
  </div>
</template>
...
export default class Search extends Vue {
  ...
  get pokemons(): Pokemon[] {
    return this.pokemonsStore.items;
  }
  ...
  getSearch(filter: string) {
    ...
    if (filter.trim().length >= 3) {
      ...
      this.pokemonsStore.fetchFilterItem(resultSearch);
    }
  }
  ...
  getPokemon(pokemon: Pokemon) {
    this.pokemonsStore.fetchItem(pokemon.name, false);
    this.showModal = true;
  }
